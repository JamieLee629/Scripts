...
#system("C:\\Program Files (x86)\\TBA\\Acceptance\\tbaacpt.exe")
...
@AARC_Combined = Prsn::AARC_COMBINED
...
unless @AARC_Combined.nil?
     ...
     enter_info.text_field(:class => "Edit", :index => 6).set "#{@AARC_Combined}"
...
reason_selection = RAutomation::Window.new :title => "Reason Selection"
person_selector.send_keys :enter if reason_selection exists?

person_data = RAutomation::Window.new :title => "Person Data"

event_selector = RAutomation::Window.new :title => "Event Selector"
event_selector.text_field(:class => "Edit", :index => 1).set "#{@effdt}"
